...
# array information
SDRF <- readRDS(".../Microarray data_sdrf.rds")

# loading patientdata
library(readxl)
Whole <- read_excel(".../pheno.xlsx")
...
annot.info <- read.csv(".../GPL20844-27159.csv",
    header = TRUE,
...
save.image(file = ".../before svn.RData")
...
save(sv_snmNorm_data, file = ".../NormalizedData.rdata")
write.csv(phenoSv, file = ".../Sample info for NormalizedData.csv")
save(phenoSv, file = ".../Sample info for NormalizedData.rdata")
...
## Read phenotype data
load(".../Sample info for NormalizedData.rdata")
Pheno <- phenoSv
...
write.csv(OutputGenes1,
    file = ".../Differential expression analysis result.csv",
    row.names = FALSE
)
save(OutputGenes1, file = ".../Differential expression analysis result.rdata")
...
load(".../Differential expression analysis result.rdata")
Genes <- OutputGenes1
rm(OutputGenes1)
######################
### Volcanol plot ####
...
write.csv(Genes,
    file = ".../DEG volcano.csv",
    row.names = FALSE
)
save(Genes, file = ".../DEG volcano.rdata")
...
ggsave(".../DEG volcano.pdf", width = 5.5, height = 5, units = "in")
dev.off()


# save all data -----------------------------------------------

save.image(file = ".../gene expression analysis.rdata")
